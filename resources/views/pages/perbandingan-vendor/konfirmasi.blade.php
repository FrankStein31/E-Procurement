                    ...
                    <div class="flex justify-between">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ $perbandingan_harga_vendor->perbandinganHarga->judul }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Nomor:
                                {{ $perbandingan_harga_vendor->perbandinganHarga->nomor }}</p>
                        </div>
                        <span
                            ...
                            @elseif($perbandingan_harga_vendor->status_penawaran === 'ditolak') bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 @endif">
                    ...
                    <div class="grid md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <p class="text-gray-900 dark:text-white">
                                {{ $perbandingan_harga_vendor->perbandinganHarga->user->name }}</p>
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 dark:text-white">
                                {{ $perbandingan_harga_vendor->batas_waktu_penawaran?->format('d/m/Y H:i') ?? '-' }}</p>
                            ...
                            <tbody>
                                @foreach ($perbandingan_harga_vendor->perbandinganHarga->pengajuanDetail as $index => $detail)
                                    <tr class="border-t">
                                        ...
                                        <td class="px-4 py-2">{{ $detail->spesifikasi }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    @if ($perbandingan_harga_vendor->status_penawaran === 'pending')
                        ...
                        @else
                            ...
                            <div
                                ...
                                <h4 class="text-lg font-semibold mb-4 text-blue-800 dark:text-blue-200">Konfirmasi
                                    Partisipasi</h4>
                        ...
                        <div id="modalSetuju"
                            ...
                            <div class="bg-white dark:bg-gray-800 p-6 rounded shadow w-96">
                                ...
                                <form method="POST"
                                    action="{{ route('perbandingan-harga.proses-konfirmasi-undangan', $perbandingan_harga_vendor->id) }}"
                                    class="mt-4 flex justify-end gap-3">
                                    @csrf
                                    <button type="submit" name="aksi" value="setuju"
                                        class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Ya,
                                        Setuju</button>
                                    <button type="button"
                                        onclick="document.getElementById('modalSetuju').classList.add('hidden')"
                        ...
                        <div id="modalTolak"
                            ...
                            <div class="bg-white dark:bg-gray-800 p-6 rounded shadow w-96">
                                <h3 class="text-lg font-semibold mb-4">Konfirmasi</h3>
                                <p>Apakah Anda yakin ingin <strong>MENOLAK</strong> undangan ini? Tindakan ini tidak
                                    dapat dibatalkan.</p>
                                <form method="POST"
                                    action="{{ route('perbandingan-harga.proses-konfirmasi-undangan', $perbandingan_harga_vendor->id) }}"
                                    class="mt-4 flex justify-end gap-3">
                                    ...
                                    <button type="submit" name="aksi" value="tolak"
                                        class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Ya,
                                        Tolak</button>
                                    <button type="button"
