                    ...
                    @foreach ($list_vendor as $nama_vendor => $list_barang)
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <input type="radio"
                                    id="barang-{{ $barang->id }}-{{ str_replace(' ', '', strtolower($nama_vendor)) }}"
                                    name="barang[{{ $barang->id }}]" @checked(old('barang.' . $barang->id) == $list_barang[$barang->id]['perbandingan_vendor_id'])
                                    value="{{ $list_barang[$barang->id]['perbandingan_vendor_id'] }}"
                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 {{ empty($list_barang[$barang->id]['total_harga']) || $list_barang[$barang->id]['total_harga'] == 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                    @disabled(empty($list_barang[$barang->id]['total_harga']) || $list_barang[$barang->id]['total_harga'] == 0) />

                                <label
                                    for="barang-{{ $barang->id }}-{{ str_replace(' ', '', strtolower($nama_vendor)) }}">
                                    <p class="text-gray-900">
                                        {{ number_format($list_barang[$barang->id]['total_harga']) }}</p>
                                    <span
                                        class="text-gray-400 text-xs">{{ '@' . number_format($list_barang[$barang->id]['harga_satuan']) }}</span>
                                </label>
                            </div>
                        </td>
                    @endforeach
